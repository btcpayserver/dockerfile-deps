...
# Preserve the configuration and database initialization used by existing images.
...
// PHP 8.1+ throws by default; keep the explicit connection retry/error handling below.
...
if (!$mysql->query('CREATE DATABASE IF NOT EXISTS `' . str_replace('`', '``', $dbName) . '`')) {
