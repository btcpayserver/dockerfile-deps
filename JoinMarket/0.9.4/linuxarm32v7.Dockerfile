...
FROM arm32v7/python:3.9.7-slim-bullseye as common
...
FROM common as cryptographybuilder

RUN apt-get install -qq --no-install-recommends libssl-dev libffi-dev rustc
...
RUN apt-get install -qq --no-install-recommends curl tini sudo procps vim git iproute2 \
    automake pkg-config libtool libgmp-dev libltdl-dev python3-dev virtualenv python3-pip supervisor
